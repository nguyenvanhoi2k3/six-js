...
describe("six.to - onScroll", () => {
  ...
  it("defaults onScroll.trigger to the tween's own target and pauses it until synced", () => {
    const a = el();
    a.style.opacity = "0";
    mockRect(a, 0, 100);
    ...
    const tween = six.to(a, { opacity: 1, duration: 1, ease: "none", onScroll: { sync: true } }) as Tween;
