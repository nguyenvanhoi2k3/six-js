...
Standalone opt-in features exported as named siblings of `six` from the package root (not methods on `six` itself) — e.g. `import { six, splitText } from "@six-js/core"`. Each plugin lives in its own kebab-case subfolder with a barrel `index.ts`; `src/plugins/index.ts` re-exports all of them and is what `src/index.ts` imports from.

- **`splitText`** (`split-text/`) — GSAP-SplitText-style splitting of an element's text into `chars`/`words`/`lines` arrays of elements. `dom-split.ts` is the pure DOM algorithm: word/char wrapping happens in-place via a recursive text-node walk (so nested tags like `<b>`/`<a>` stay exactly where they were, no ancestor-cloning needed), grapheme-aware via `Intl.Segmenter` when available. Line grouping measures each word's `getBoundingClientRect().top` to detect visual line breaks, then uses `Range.extractContents()` per line — this is what makes nested formatting survive line splitting too (the browser splits/clones any partially-included ancestor for you). `split-text.ts` holds the `SplitText` class: every (re-)split restores from the originally-saved `innerHTML` first, so calling `.split()` again or `autoSplit`'s `ResizeObserver`/`document.fonts.ready`-triggered re-splits are idempotent rather than double-wrapping already-split content. `mask: "chars" | "words" | "lines"` wraps that level in an `overflow:hidden` box for reveal-style animations without changing what `.chars`/`.words`/`.lines` point to. `aria: "auto"` (default) sets `aria-label` on the root and `aria-hidden` on generated spans, but only removes attributes it set itself. `onSplit` may return a `Killable`/`Killable[]` (e.g. a `Playable` from `six.to(split.lines, {...})`) which gets killed automatically before the next re-split — this is what makes `autoSplit` + `onSplit` usable for responsive line-reveal animations instead of leaking tweens on resize. `SplitText.kill()` aliases `revert()` so instances created inside a `six.media()` handler are auto-reverted on scope teardown via the same `getActiveScope()?._capture(this)` mechanism `Playable`/`SxMediaScope` use.
